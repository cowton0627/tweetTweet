...
## 2026-08-14（Phase 5：個人頁）

### 自己的頁與別人的頁是同一個畫面

直覺會做兩個:一個「我的個人資料」,一個「使用者頁」。但它們顯示的東西一模一樣,差別只有一顆按鈕——你不能追蹤自己。兩個畫面的結果是其中一個會慢慢長出另一個沒有的東西,然後兩邊看起來就不像同一個 App。

所以是一個 `AccountView(handle:)`,「個人」分頁只是傳自己的 handle 進去。附帶的好處是登入之後那個分頁終於有內容——原本只有名字跟頭像。

**`isMe` 由伺服器回報,不是 client 比 handle。** 伺服器知道是誰在問,而 client 端的比對要處理「還沒載入完」「大小寫」「登出到一半」這些狀態,每一個都是一次出錯的機會。

### 個人頁的貼文依日期排序,不是 `position`

`position` 是「在某個 feed 裡排第幾」。同一個人在推薦與熱門各有貼文時,兩個 position 之間沒有關係,拿來排序會得到隨機的順序。個人頁是一個人的時間軸,所以依 `date DESC`。

### 共用組裝流程時抓到一個漏掉的 join

feed 查詢與個人頁查詢只差一個 WHERE,所以把「查貼文 → 查圖片 → 查最新回文 → 組起來」抽成共用。抽的當下才發現圖片那個查詢只 join 了 `posts`,而共用之後條件會提到 `u.handle`——`WHERE u.handle = ?` 在一個沒有 `users` 的查詢裡。

這種錯誤在「複製貼上改一改」的寫法裡不會浮現,因為兩份查詢各自都是對的。共用把隱含的假設變成明講的參數,順手把它逼了出來。

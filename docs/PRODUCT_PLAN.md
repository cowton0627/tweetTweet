...
### 回文的互動：不要 action sheet ✅ 已實作

原本按「回應」跳出一個 sheet，這在 FB／Threads 都不是這樣。
...
實際與規劃的差異：
- **分頁游標用 comment id，不是 offset。** 有人在讀的同時也有人在寫，offset
  會漏掉或重複一則。往回翻（`before`）但由舊到新顯示——這就是一串對話真正的
  樣子:你從最新的到達,然後往過去走。
- **feed 的那兩則用 window function 一次查完**，不是每則貼文查一次。二十則
  貼文因此是一個 query 而不是二十個。
- **刪除已經不在的回文回 404**，跟「取消已經沒按過的讚」不同。後者在描述一個
  狀態,前者在指名一個列;默默成功會蓋掉 client 拿著過期清單這件事。
- **刪別人的回文回 403 不是 404**,因為那則回文就在畫面上,裝作不存在騙不了人,
  而且也沒有洩漏什麼——本來就看得到。
- **種子貼文的 `commentCount` 保留原本的假數字**（例如 2200）,跟 `likeCount`
  一樣。真的回文疊加在上面。

**Phase 5 — 個人頁** ✅ 已完成（通知未做）
個人頁（自己／別人、追蹤按鈕、該使用者的貼文）。通知需要另一份設計，仍未做。

實際與規劃的差異：
- **自己的頁跟別人的頁是同一個畫面。** 兩個畫面會漸行漸遠，而它們的差別只有
  一顆按鈕——你不能追蹤自己。`isMe` 由伺服器回報，不是 client 自己比對 handle
  （那種比對很容易在某個邊界上出錯）。
- **個人頁的貼文用日期排序，不是 `position`。** position 是「在某個 feed 裡
  的位置」，同一個人在兩個分類的貼文 position 互不相干。
- **feed 查詢與個人頁查詢共用組裝流程**，兩者只差一個 WHERE。順帶修掉一個
  漏掉的 join:圖片查詢原本沒有 join `users`，共用之後條件會提到 `u`。

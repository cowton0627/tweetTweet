...
- 個人頁：帳號資訊、貼文數／追蹤者／追蹤中、追蹤按鈕，以及該帳號的所有貼文（跨兩個分類、依日期排序）。自己的頁與別人的頁是同一個畫面，差別只有一顆按鈕。點貼文上的作者即可開啟。
- 63 個 XCTest、186 個後端測試，以及 GitHub Actions build/test 流程。
...
[`PRODUCT_PLAN.md`](PRODUCT_PLAN.md) 的 Phase 1–5 已完成，剩下的是原本就標為
獨立設計或已知問題的項目：

1. 圖片上傳前降取樣。
2. 站內通知（需要另一份設計）。
3. 依 accessibility 清單完成實機 VoiceOver 測試並記錄裝置與 iOS 版本。
4. 刪除回文改到詳情頁之後留下的無用檔案。

...
- 40 個 XCTest、144 個後端測試，以及 GitHub Actions build/test 流程。
...
1. **Phase 3**：讚與追蹤移到 `likes`／`follows` 關聯表，feed 查詢帶入 viewer。
2. **Phase 4**：回文（`comments` 表、詳情頁列表、底部輸入列），並取代目前的 action sheet。
3. **Phase 5**：個人頁看別人的貼文與追蹤按鈕。
4. 圖片上傳前降取樣。
